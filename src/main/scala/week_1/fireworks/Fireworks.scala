...
object Firework:
  ...
  def next(firework: Firework): Firework =
    firework match {
      case Done => Done
      case waiting: Waiting => Launched.init(Point(1,2), 2, Color.blue) // random values
      case launched: Launched => Exploding(1, Particles(List(Particle(1, 2, Point(2,3), Color.blue)))) // random values
      case exploding: Exploding => Done
    }
...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework :
  ...
  def next: Firework =
    if countDown > 0 then
      val newPosition: Point = Motion.movePoint(position, direction, Settings.propulsionSpeed)
